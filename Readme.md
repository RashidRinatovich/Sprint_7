...
│
├── data/
│   ├──courier_generators          # Тестовые данные для курьера                    
│   ├── order_test_data.py         # Статус коды      
│   └── order_generators.py        # Генератор данных для заказов
│   ├── test_data.py               # Общие тестовые данные
├── helpers/
│   ├── courier_helpers.py         # Хелпер для курьеров (создать/логин/удалить)
│   └── order_helpers.py           # Хелпер для заказов (создать/получить список)
│
